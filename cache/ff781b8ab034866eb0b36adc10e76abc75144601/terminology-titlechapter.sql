-- &query& = terminology title:Chapter

-- arguments text_terms:string "'terminology'"
...
SELECT
  ...
  ts_headline(ab.html,plainto_tsquery('terminology'), 'ShortWord=5, MinWords=50, MaxWords=60') as abstract,
  -- until we actually do something with it
  -- ts_headline(mfti.fulltext, plainto_tsquery('terminology'),
...
FROM (
  ...
  'terminology'::text ||'-::-parentAuthor' as key
...
FROM (
  ...
  'terminology'::text ||'-::-language' as key
...
FROM (
  ...
  'terminology'::text ||'-::-subject' as key
...
FROM (
  ...
  'terminology'::text ||'-::-fulltext' as key,

  ts_rank_cd('{1.0,1.0,1.0,1.0}', module_idx, plainto_tsquery('terminology'),4) * 2 ^ length(to_tsvector('terminology')) as rank
...
WHERE
  ...
  module_idx @@ plainto_tsquery('terminology')
...
FROM (
  ...
  'terminology'::text ||'-::-abstract' as key
...
FROM (
  ...
  'terminology'::text ||'-::-keyword' as key
...
FROM (
  ...
  'terminology'::text||'-::-author' as key
...
WHERE
  ...
  (u.firstname ~* req('terminology'::text)

   OR

   u.surname ~* req('terminology'::text)

   OR

   u.fullname ~* req('terminology'::text)

   OR

   u.email ~* (req('terminology'::text)||'.*@')
...
FROM (
  ...
  'terminology'::text||'-::-editor' as key
...
FROM (
  ...
  'terminology'::text||'-::-translator' as key
...
FROM (
  ...
  'terminology'::text||'-::-maintainer' as key
...
FROM (
  ...
  'terminology'::text||'-::-licensor' as key
...
WHERE
  ...
  name ~* ('(^| )'||req('terminology'::text)||'( |$)'::text)
