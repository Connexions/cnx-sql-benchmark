-- &query& = author:Fred pubYear:2007
...
WHERE
  ...
  (u.firstname ~* req('Fred'::text)

   OR

   u.surname ~* req('Fred'::text)

   OR

   u.fullname ~* req('Fred'::text)

   OR

   u.email ~* (req('Fred'::text)||'.*@')
...
WHERE
  weighted.module_ident = lm.module_ident
   AND extract(year from revised) = 2007
